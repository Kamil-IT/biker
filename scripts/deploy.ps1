<#
.SYNOPSIS
  Deploys biker to GCP Cloud Run: backend (FastAPI + Chromium), frontend (nginx) and the on-demand OLX
  searcher (claude CLI + Chromium, TODO-031) as three services in europe-central2, all against Cloud SQL
  biker-pg. Run by hand from the developer machine (TODO-030).
...
if ($Only -in 'all', 'searcher') {
    # One CLI run + one Chromium per instance, scale to zero: --concurrency 1 mirrors SEARCHER_MAX_CONCURRENT=1,
    # and a search is minutes, hence the 900 s timeout (the backend waits at most SEARCHER_TIMEOUT = 600 s).
...
if ($Only -in 'all', 'backend') {
    # Without a deployed searcher the backend still works; POST /v1/bike/used/search then answers 503.
    ...
    if ($searcherUrl) {
        ...
        Write-Warning 'biker-searcher is not deployed: the backend is deployed without SEARCHER_URL (OLX search answers 503)'
    }
