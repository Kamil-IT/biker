...
| 029 | Dockerfiles + docker-compose (Postgres, backend, frontend), `PLAYWRIGHT_HEADLESS` | [#92](https://github.com/Kamil-IT/biker/pull/92) |
| 031 | On-demand OLX searcher service (`searcher/`, Claude Code CLI on the subscription, Cloud Run) + DB-only `/v1/bike/used` and `POST /v1/bike/used/search` behind the Used card's Request-data button | [#97](https://github.com/Kamil-IT/biker/pull/97) |
| 032 | Decathlon search moved into the same searcher (`POST /v1/search/decathlon`) + DB-only `/v1/bike/decathlon` and `POST /v1/bike/decathlon/search` (house-brand allowlist, foreign brands skipped without a run) behind the New card's Request-data button | [#99](https://github.com/Kamil-IT/biker/pull/99) |
| ISSUE_010 | Decathlon offers always empty for non-Decathlon brands — closed by the allowlist skip in 032 | [#99](https://github.com/Kamil-IT/biker/pull/99) |
...
**009 adds a fourth failure mode: a test that never runs at all.** Its 43 parser unit tests lived in a new `backend/tests/`, but `pytest.ini` pins `addopts` to an explicit path list, so a bare `pytest` never collected them — they passed only when invoked with an explicit path, which is exactly how their author ran them. A suite that is green because it is empty looks identical to one that is green because it passes. After adding a test directory, run `pytest --collect-only` and confirm your file is in the list. The same line then produced the merge conflict with 018: both branches appended to `addopts`, and taking either side alone would have silently dropped the other's suite.

**009 also left a known, unfixed hazard.** The AI fallback ends with `save_search()`, and the new DB-first branch reads that table — so when the model invents a bike for a brand+model that does not exist, the fabrication is persisted and served confidently as a cache hit for 24 h, with no marker distinguishing it from a real result. Reproduced twice with `{"brand":"Zzyzx","model":"Nonesuch QQ999"}`. `save_search` on the AI path predates 009 and the TTL bounds it, but 009 widened reachability from one byte-identical request to any request for that brand+model. A follow-up task is pending.
