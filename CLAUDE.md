...
`docker compose up --build -d` → http://localhost:8080 (db on 5433, backend on 8000, nginx frontend on 8080, searcher on 8100 — the backend reaches it as `SEARCHER_URL=http://searcher:8100`; the searcher container needs `CLAUDE_CODE_OAUTH_TOKEN` in `searcher/.env`). Load data once with
