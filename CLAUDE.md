...
This mirrors the pattern used by `/v1/bike/allegro`. (`/v1/bike/used/olx` and `/v1/bike/decathlon` no longer call the API at all — they are DB reads of `bike_offer`, TODO-031 / TODO-032; the searches behind them run in the searcher service via `/v1/bike/used/search` and `/v1/bike/decathlon/search`, which are never cached.)
...
**Endpoint** `POST /v1/bike/allegro` (renamed from `/v1/bike/offer`; the generic-cache key stays `/v1/bike/offer` so existing rows still hit)
- Request: `{"company": "Canyon", "model": "Grizl CF 7 ESC"}`
...
- `POST /v1/bike/allegro` `{ "company": "...", "model": "..." }` → `{ offers: BikeOffer[], info: string }` (allegro.pl)
