...
This mirrors the pattern used by `/v1/bike/offer`. (`/v1/bike/used` no longer calls the API at all — it is a DB read, TODO-031.)
...
- `/searcher` — on-demand OLX used-bike searcher (TODO-031): a small FastAPI service (port 8100) that runs the **Claude Code CLI** (`claude -p`, billed to the subscription via the OAuth login / `CLAUDE_CODE_OAUTH_TOKEN`, no `ANTHROPIC_API_KEY`) with the OLX prompt, scrapes listing photos with Playwright and writes the offers into the shared `bike_offer` / `bike_offer_photos` tables. Called only by the backend's `POST /v1/bike/used/search` (shared secret header `X-Searcher-Key`) or by hand with `curl`. Meant for a scale-to-zero Cloud Run service; see `searcher/README.md`
...
copy .env.example .env   # then edit .env: ANTHROPIC_API_KEY, DATABASE_URL to use Postgres, SEARCHER_URL + SEARCHER_API_KEY for the OLX searcher (unset → /v1/bike/used/search answers 503)
...
`docker compose up --build -d` → http://localhost:8080 (backend on 8000, nginx frontend on 8080, OLX searcher on 8100 — the backend reaches it as `SEARCHER_URL=http://searcher:8100`; the searcher container needs `CLAUDE_CODE_OAUTH_TOKEN` + `SEARCHER_API_KEY` in `searcher/.env` and mounts the same pgpass file). The database is **GCP Cloud SQL**
...
| Stored OLX offers | `app/offers_repository.py` `get_used_offers` · `bike_exists` (both on `repository._find_bike_id`) | TODO-031: reads `bike_offer` rows with `source = 'olx.pl'` (`ORDER BY id`) + `bike_offer_photos` (`display_order`) for the Python-normalised bike; `brand`/`model` on each offer come from the `bike` row (the table has no title column), `is_new=False`, `info=""`. Unknown bike / no rows / DB error → empty response, never an error. No TTL. Rows are written only by the searcher service |
| Searcher client | `app/searcher_client.py` | `search_olx(company, model)` — httpx `POST {SEARCHER_URL}/v1/search/olx` with header `X-Searcher-Key` (`SEARCHER_API_KEY`), timeout `SEARCHER_TIMEOUT` (600 s, connect 10 s). Single-flight per normalised `(company, model)` (a second click joins the running search; the task is `asyncio.shield`ed so a disconnect does not cancel it) and at most `SEARCHER_MAX_INFLIGHT` (1 — never more than the searcher's `SEARCHER_MAX_CONCURRENT`) distinct searches at once; a searcher 503 (its slot taken) is `SearcherBusy` too — nothing queues. Raises `SearcherNotConfigured` (URL or key unset) / `SearcherUnavailable` / `SearcherBusy` → 503 (fixed detail strings, the exception text only in the log), `SearcherFailed(status, detail)` → 502 with the searcher's detail (≤ 300 chars). The searcher's `bike_id`/`saved` fields are dropped |
...
| App shell | `src/App.tsx` | View router (`search` / `details` / `equipment`), search, details, review, allegro offer, ceneo offer, decathlon offer & used-bike state, plus equipment details/review state; all API calls. `searchUsedBikes` (TODO-031) POSTs `/v1/bike/used/search`, throws on non-OK, and writes the offers into `usedBikes` **without** flipping `usedBikeState` to `loading` (the button shows its own spinner); a `selectedBikeRef` guard drops a result that lands after the user opened another bike. A free-text-only submit first calls `/v1/bike/parse`: extracted fields populate the Filters panel and the search waits for a second submit, while a **400** renders a "Not found" warning above the search box and stops there. Clicking a component element name in a bike's spec tree opens the equipment view for that item |
