...
| Follow-up cache | `app/store.py` | Additive queryable tables `search_cache` (24 h TTL, keyed by normalised enriched query) and `bike_details_cache` (30 d TTL, keyed by company+model) in the same `cache.db`; typed helpers `save_search`/`get_search_by_query`/`find_bikes_by_brand`/`save_bike_details`/`get_bike_details`; powers the cache-only follow-up GET endpoints. Also `find_bike_by_brand_model(brand, model)` (fresh `search_cache` rows matching brand **and** model) and `find_offer_prices(brand, model)` (every parseable price across the four offer endpoints' cached responses, joined on `cache.py`'s `_normalise({company, model})`) — together they back the DB-first branch of `POST /v1/bike/search` |
