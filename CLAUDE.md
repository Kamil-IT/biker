...
| Citation chips | `src/components/CitationChips.tsx` | Google AI Overview-style source chips: one clickable, terracotta-tinted domain badge per citation (deduplicated by URL), full URL shown as `title` tooltip; used by the Overview and Expert Review blocks |
| Details view | `src/components/BikeDetailsView.tsx` | Full spec sheet: back nav, bike header, Overview (DescriptionCard — flowing text + `CitationChips` row), Allegro Offers (OffersSection), Ceneo Offers (OffersSection), Used Bikes OLX (UsedBikesSection), Expert Review (ReviewSection — explanation + `CitationChips` row from `ref[]`), category/subcategory/element/spec tree, shimmer skeleton, error + retry |
