...
async def _find_product_url(company: str, model: str) -> str:
    system_prompt = (PROMPTS_DIR / "equipment_photos.md").read_text(encoding="utf-8")
    ...
    try:
        response = await _client.messages.create(
            ...
            messages=[{"role": "user", "content": f"Find the official product page URL for the {item} cycling equipment item on the manufacturer's website."}],
        )
    except Exception as exc:
        logger.error("equipment photos url-search error | %s", exc)
        return ""
...
def _scrape_images_sync(url: str) -> list[str]:
    ...
    logger.info(
        "equipment photos scraped | url=%s count=%d elapsed=%.2fs",
        url, len(result), time.perf_counter() - t,
    )
    return result


async def find_equipment_photos(company: str, model: str) -> list[str]:
    product_url = await _find_product_url(company, model)
    if not product_url:
        logger.info("equipment photos: no product URL found for %s %s", company, model)
        return []

    logger.info("equipment photos: scraping %s", product_url)
    try:
        photos = await asyncio.to_thread(_scrape_images_sync, product_url)
    except Exception as exc:
        logger.error("equipment photos scrape error (non-fatal) | %s", exc)
        return []
