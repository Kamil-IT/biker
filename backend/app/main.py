...
from .bike_offers_finder import find_generic_offers  # noqa: E402
...
@app.post("/v1/bike/offers", response_model=BikeOfferResponse)
async def bike_offers(req: BikeOfferRequest) -> BikeOfferResponse:
    logger.info("generic offers request | company=%r model=%r", req.company, req.model)
    _fields = {"company": req.company, "model": req.model}
    cached = get_cached("/v1/bike/offers", _fields, BikeOfferResponse)
    if cached is not None:
        return cached

    t_start = time.perf_counter()
    result = await find_generic_offers(req.company, req.model)
    elapsed = time.perf_counter() - t_start
    logger.info("generic offers complete | offers=%d elapsed=%.2fs", len(result.offers), elapsed)
    if result.offers:
        set_cached("/v1/bike/offers", _fields, result)
    return result
