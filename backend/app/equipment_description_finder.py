...
logger = logging.getLogger("biker.equipment.description")
...
async def find_equipment_description(company: str, model: str) -> BikeDescription:
    system_prompt = (PROMPTS_DIR / "equipment_description.md").read_text(encoding="utf-8")
    ...
    response = await _client.messages.create(
        ...
        messages=[{"role": "user", "content": f"Describe the {item} cycling equipment item in 4–5 sentences."}],
    )
    ...
    if not text:
        logger.error("no text in equipment description response")
        return _FALLBACK

    logger.info(
        "equipment description done | elapsed=%.2fs input_tokens=%d output_tokens=%d segments=%d citations=%d",
        elapsed,
