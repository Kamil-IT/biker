...
def _seed_offer_rows_if_missing(company: str, model: str, by_endpoint: dict) -> list[str]:
    ...
    try:
        for endpoint, prices in by_endpoint.items():
            if conn.execute(
                "SELECT 1 FROM endpoint_req_to_body_cache WHERE endpoint = ? AND request = ?", (endpoint, key)
            ).fetchone() is not None:
                continue
            response = {
                "offers": [
            ...
            conn.execute(
                "INSERT INTO endpoint_req_to_body_cache "
...
def _drop_offer_rows(company: str, model: str, endpoints: list[str]) -> None:
    ...
    try:
        for endpoint in endpoints:
            conn.execute(
                "DELETE FROM endpoint_req_to_body_cache WHERE endpoint = ? AND request = ?", (endpoint, key)
            )
        conn.commit()
    finally:
        conn.close()
...
def _joined_prices(company: str, model: str) -> list[float]:
    ...
    try:
        rows = conn.execute(
            "SELECT response FROM endpoint_req_to_body_cache WHERE endpoint IN (?, ?, ?, ?) AND request = ?",
            (*OFFER_ENDPOINTS, key),
        ).fetchall()
    finally:
        conn.close()
