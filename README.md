...
optional OLX / Decathlon searcher, see Terminal 3).
...
### Terminal 3 — OLX / Decathlon searcher (optional, TODO-031 / TODO-032)

The used-bike search on OLX and the new-bike search on decathlon.pl run in their own service so they only spend tokens
when a user asks for them — and they spend them from the **Claude Code subscription** (`claude -p`), not from
`ANTHROPIC_API_KEY`. Log the CLI in once (`claude`), then:
...
Both routes share one busy slot (one CLI run per instance); the second call answers 503 `searcher busy` while the first
runs. The Decathlon search stores no photos.
...
| `searcher` | `searcher/Dockerfile` | 8100 | Python 3.14 + Node 24 + `@anthropic-ai/claude-code` (pinned) + patchright Chromium; runs `claude -p` with `CLAUDE_CODE_OAUTH_TOKEN` for both the OLX and the Decathlon search, writes `bike_offer` into the same database (same pgpass mount); the backend reaches it as `SEARCHER_URL=http://searcher:8100` |
...
TODO-031 adds a third service, `biker-searcher` (the on-demand OLX searcher; TODO-032 adds the Decathlon search to the
**same** service — same image, secrets and busy slot, no new service): same Cloud SQL socket, `--concurrency 1`,
...
| AI | Anthropic Claude Haiku (`claude-haiku-4-5-20251001`) — API key for the backend; the searcher (OLX + Decathlon) calls the same model through the Claude Code CLI (`claude -p`, subscription OAuth token) |
...
│   │   ├── offers_repository.py       # Stored marketplace offers read side: get_used_offers (olx.pl) + get_decathlon_offers (decathlon.pl) + bike_exists
│   │   ├── searcher_client.py         # httpx proxy to the searcher (search_olx / search_decathlon), single-flight + shared in-flight cap
...
└── frontend/
    └── src/
        ...
        └── components/
            ...
            ├── RequestDataButton.tsx  # "Request data" button for empty bike-details sections (POST /v1/bike/missing); in the Used card also runs the OLX search, in the New card the Decathlon search
...
└── searcher/                          # On-demand OLX + Decathlon searcher (TODO-031 / TODO-032) — FastAPI on :8100, Claude Code CLI + Playwright
    ├── app/
    │   ├── main.py                    # POST /v1/search/olx + POST /v1/search/decathlon (X-Searcher-Key, one shared busy slot) + GET /health
