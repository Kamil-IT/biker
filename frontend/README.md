...
| `POST /v1/bike/review` · `/offer` · `/ceneo` · `/decathlon` | `{ company, model }` | review / offers |
...
| `POST /v1/bike/used/search` | `{ company, model }` | `UsedBikeResponse` `{ offers, info }` — on-demand OLX search via the searcher service (TODO-031), triggered only by the Used card's "Request data" button; 503 when the searcher is not configured/reachable, 502 when it fails |
