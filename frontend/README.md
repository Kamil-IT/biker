...
| `POST /v1/bike/allegro` | `{ company, model }` | `BikeOfferResponse` `{ offers, info }` — stored allegro.pl offers only, with their photos (DB read, no AI, TODO-033); empty until the on-demand search below has run |
