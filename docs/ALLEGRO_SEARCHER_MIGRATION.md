...
| Zdjęcia dalej Playwrightem (rozgrzewka DataDome + regex)? | **Tak**, „dokładnie to samo, inny transport”; ryzyko blokady DataDome przyjęte świadomie (scrape nie-fatalny, `photos: []`). **Decyzja zmieniona po sondach** (§ 3): scrape wyleciał w całości, Allegro nie ma zdjęć z założenia |
...
| 4. Trek Marlin 6 przez **prawdziwy** `find_allegro_offers` (prompt + regex URL + ówczesny fetcher) | 3 oferty z cenami (2 799–3 458 zł) w 93 s; obie strony ofert **403** → pominięte, `photos: []` |

Zdjęcia najpierw zostały krokiem nie-fatalnym „na wypadek, gdyby Allegro odpuściło” — i tak przeszły przez workflow oraz
testy ręczne (każdy prawdziwy run: 0 zdjęć, ~10 s i jedno uruchomienie Chromium w plecy). **Druga decyzja po sondach
(użytkownik, 2026-09-26): „wywal te zdjęcia z implementacji i searcha, nie ma co marnować tokenów”.** Skoro allegro.pl
odpowiada 403 każdemu automatowi, scrape nie miał czego przynieść — `searcher/app/allegro_image_fetcher.py` skasowany bez
zamiennika, `allegro_finder.py` nie uruchamia przeglądarki, `photos` zawsze `[]` (tak jak Decathlon), Playwright w
searcherze pracuje wyłącznie dla OLX. To nie regresja wobec starego backendu — tam też ponad połowa rowerów miała 0 zdjęć.
...
- `app/allegro_finder.py` — `find_allegro_offers()`: schemat OLX, ≤ 3 ofert, URL musi pasować do
  `^https://(www\.)?allegro\.pl/(oferta|produkt)/` (strona listingu/kategorii byłaby zapisana jako „oferta”), dedupe,
  przycięcie do szerokości kolumn, `is_new` z modelu, `city=None`, `photos=[]` zawsze — **bez scrape'u zdjęć i bez
  Playwrighta** (fetcher wyleciał decyzją po sondach, § 3; trasa jest CLI-only jak Decathlon).
- `app/repository.py` — `save_offers(source='allegro.pl')` bez zmian w kodzie (upsert w obrębie (rower, źródło), pusty
  wynik nic nie kasuje; dla Allegro nie powstają wiersze `bike_offer_photos`).

**Backend**
- `/v1/bike/allegro` → `offers_repository.get_allegro_offers()` (czysty odczyt; `photos` zawsze `[]`, bo wiersze Allegro
  nie mają zdjęć); koniec z `_ALLEGRO_CACHE_KEY`.
...
- Usunięte: `bike_offer_finder.py` (→ searcher, przepisany), `allegro_image_fetcher.py` (**bez zamiennika** — scrape
  zdjęć skasowany), `prompts/bike_offer_allegro.md` (→ searcher,
...
| moje (po testach ręcznych) — fetcher zdjęć Allegro przeszedł przez workflow, przeglądy i testy, choć w żadnym runie nie przyniósł zdjęcia (403), a kosztował ~10 s i start Chromium | decyzja użytkownika: scrape wycięty — `searcher/app/allegro_image_fetcher.py` skasowany, `allegro_finder.py` bez Playwrighta, `photos: []` z założenia; `case_allegro` seeduje wiersz bez zdjęcia i asercja `photos == []` |
...
| low — `ALLEGRO_URL_PREFIXES` przepuszczało `allegro.pl/listing?string=…` (prompt sam taki URL budował) → strona wyszukiwania zapisana jako „oferta” | regex `/(oferta|produkt)/` w finderze + zasada w prompcie |
...
2. **Smoke backendu** (`test_search.py` przez `BIKER_API_URL`): 9 passed / 0 failed / 3 skipped — `case_allegro` 0,7 s z
   fixtury (wiersz bez zdjęcia — po wycięciu scrape'u asercja to `photos == []`; bez wiersza cache), `case_allegro_search` 404 w 0,3 s, jedyny płatny `case_decathlon_search`
...
3. **`/manual-tester`**: plan ISTQB, 9 przypadków, dwa stacki — prawdziwy oraz fałszywy searcher na 8199 wybierający
   ...
   - TC-01 Trek Marlin 6: przycisk po 5 s → „Szukam na Allegro i Decathlon…” → Decathlon natychmiast pusto (obca marka),
     ...
     0 zdjęć (403 — ten run był ostatnim argumentem za wycięciem scrape'u, § 3). TC-02: ponowne otwarcie — odczyt z bazy
     w 2 s, bez wyszukiwania, bez przycisku.
...
Jeden commit `df8ae5b` (32 zmienione pliki + nowe: finder, fetcher — skasowany zaraz potem na tym samym branchu, § 3 —, prompt, plan testów z 13 zrzutami, ten dokument i
...
przeglądarka ──klik „Poproś o dane” (karta Nowe)──► frontend (nginx / Vite)
   ...
   │                                   │          │ bez Playwrighta (Allegro i Decathlon) — Chromium tylko dla OLX; photos: []
   │                                   │          ▼
   │                                   │   Cloud SQL biker-pg: bike_offer (source='allegro.pl' / 'decathlon.pl'), bez wierszy bike_offer_photos
   │                                   │          ▲
   └── automatycznie przy otwarciu roweru: POST /v1/bike/allegro ── czysty odczyt ──┘
```

Pliki: `searcher/app/allegro_finder.py` (nowy, bez scrape'u zdjęć), `searcher/app/prompts/bike_offer_allegro.md`
(przeniesiony, przepisany pod CLI),
...
`bike_offer_finder.py`, `allegro_image_fetcher.py` (bez zamiennika — § 3), `prompts/bike_offer_allegro.md`,
...
- **Zdjęcia Allegro — nie ma ich z założenia** (decyzja użytkownika po sondach, § 3): allegro.pl odpowiada 403 każdemu
  automatowi (CLI `WebFetch` i Chromium, headless i z oknem), więc scrape nie miał czego przynieść, a kosztował ~10 s i
  start przeglądarki na run. Fetcher skasowany bez zamiennika; `photos: []` jak w Decathlonie. Gdyby zdjęcia miały kiedyś
  wrócić, jedyna sensowna droga to oficjalne API (TODO-008, wymaga zweryfikowanej aplikacji), nie kolejne sztuczki z
  przeglądarką.
...
5. **Zewnętrzny serwis, który blokuje boty, nie jest błędem implementacji** — ale trzeba to udowodnić (status, tytuł,
   markery DataDome, oba tryby przeglądarki) i wtedy **wyciąć martwy krok, a nie zostawiać go „na wszelki wypadek”**:
   fetcher, który w każdym runie kosztuje ~10 s i start Chromium, a przynosi 0, to czysta strata — użytkownik ujął to
   krótko: „nie ma co marnować tokenów”.
