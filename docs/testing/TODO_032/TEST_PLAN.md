...
| R8 | Smoke tests: `backend/scripts/test_search.py` TC-33..35 (after merging `main` — PR #100 restructured the suite into one `case_*` per endpoint — they live on as `case_decathlon` / `case_decathlon_search`), `searcher/scripts/test_searcher.py` TC-7..9 | `test_search.py` `case_decathlon` / `case_decathlon_search`, `test_searcher.py:128/138/159`; `BikeOfferRequest` bounded to 255 chars (`schemas.py:252`) | Implemented — results below |
...
Post-merge re-run (2026-09-26, after `main`'s PR #100 restructured `test_search.py` into one `case_*` per endpoint and the Decathlon cases were ported as `case_decathlon` / `case_decathlon_search`): `BIKER_API_URL=http://127.0.0.1:8001 python scripts/test_search.py` → **7 passed, 0 failed, 3 skipped** (the three `--ai` cases) — `used_search` 74 s, `decathlon_search` 166 s, both live through the local searcher.
