...
prompt, DB write; no photo scrape — dropped after the probes, see K2), the backend's DB-only `POST /v1/bike/allegro`, the proxy
...
- **allegro.pl answers HTTP 403 (DataDome) to every automated request** from this machine — the CLI's WebFetch and
  Chromium alike (probes, see the task file decision 4). During this run the Allegro route still carried a non-fatal
  Playwright photo scrape, which returned 0 photos every time; **the user then decided to drop the scrape entirely**
  (nothing to gain, ~10 s and a browser launch wasted per run), so the shipped code stores Allegro offers **without
  photos by design** — `photos: []`, no `bike_offer_photos` rows, Playwright only for OLX. The rows the real searches
  stored below are identical to what the shipped code stores.
...
|---|---|---|---|
| R1 | `POST /v1/bike/allegro` is a pure DB read of `bike_offer` (`source='allegro.pl'`): zero AI/CLI calls, no generic cache, `{offers, info}` unchanged, `photos` always `[]` (no photo rows by design, K2), unknown bike → 200 empty | `backend/app/main.py` `bike_allegro`, `backend/app/offers_repository.py` `get_allegro_offers` (over `_get_stored_offers`, `is_new` from the row) | Implemented |
...
| R3 | Searcher `POST /v1/search/allegro`: same auth/status mapping, ≤ 3 offers, only `allegro.pl/oferta/…` or `/produkt/…` URLs, `is_new` from the listing, **no photo scrape** (`photos: []`, no Playwright — dropped after the probes, K2), replace semantics scoped to `(bike, 'allegro.pl')`, empty result keeps rows | `searcher/app/main.py` `search_allegro` → `_run_search`, `searcher/app/allegro_finder.py` (`ALLEGRO_OFFER_URL_RE`, `_to_offers`, `photos=[]`), `searcher/app/repository.py` `save_offers` (unchanged) | Implemented |
...
| R6 | `backend/app/bike_offer_finder.py`, `allegro_image_fetcher.py` and `prompts/bike_offer_allegro.md` leave the backend (the finder and prompt are rewritten in the searcher; the image fetcher is deleted without a replacement — K2); the dev scripts that only drove them go; `pytest` passes | deleted (`git status`); `scripts/test_browser_slots.py` no longer imports them (and no longer imports the long-gone `olx_image_fetcher` — see X4); `pytest` 22 passed | Implemented |
| R7 | Smoke tests: `backend/scripts/test_search.py` `case_allegro` (seeded allegro.pl fixture without a photo row, `photos == []`, < 5 s, no generic-cache row, unknown bike → 200 empty) + `case_allegro_search` (unknown bike → 404, no paid run); `searcher/scripts/test_searcher.py` TC-7 (401) / TC-8 (422) on `/v1/search/allegro`; the one paid run of the suite stays `case_decathlon_search` | `test_search.py` `case_allegro` / `case_allegro_search`, `test_searcher.py` TC-7/8 | Implemented — results below |
...
| X1 | (decided during the probes, task decision 4) the prompt is **not** byte-identical: allegro.pl answers 403 to every fetch, so `searcher/app/prompts/bike_offer_allegro.md` is a CLI-tuned rewrite that works from WebSearch results only — price/condition from the result snippet, `price: ""` when none shows it | `searcher/app/prompts/bike_offer_allegro.md`; probes: Kross Level 3.0 → 3 offers / 67 s, Trek Marlin 4 → 3 offers / 75 s, Trek Marlin 6 (through the real module, incl. the then-present fetcher) → 3 offers with prices / 93 s + 403 on both offer pages → `photos: []` (the finding that led to dropping the scrape, K2) | Extra — validated |
...
| K2 | No photos by design: the probes and round 1 showed allegro.pl answers 403 (DataDome) to every automated fetch — the CLI's WebFetch and Chromium alike, headless and headed — so the Playwright scrape never returned a photo; the user dropped it after the probes/round 1 ("nie ma co marnować tokenów"). Allegro offers are stored with `photos: []`, no `bike_offer_photos` rows, no browser launch; Playwright runs only for OLX | `searcher/app/allegro_finder.py` (`photos=[]`; `allegro_image_fetcher.py` deleted) | Decided — TC-033-01 |
...
- **Expected:** skeleton first, then the button; after the click the button reads "Szukam na Allegro i Decathlon…" with a
  ...
  (by `is_new`), the button gone; `bike_offer` holds the rows (`source='allegro.pl'`, no photo rows — none stored by
  design, K2);
...
- `POST /v1/bike/allegro` seeded fixture read (`is_new` true, `city` null, `photos == []`, no generic-cache row,
...
- Searcher module end to end without the server (probe, real CLI run, fetcher still present at the time): Trek Marlin 6
  → 3 offers with prices in 93 s, both offer pages 403 → `photos: []` (the scrape was removed after this; the shipped
  module stores the same rows without launching a browser).
...
- `backend/scripts/test_search.py` (`BIKER_API_URL=http://127.0.0.1:8002`) → **9 passed, 0 failed, 3 skipped** (the three
  `--ai` cases): `allegro` 0.7 s (fixture read — at the time seeded with a photo row; after the scrape was dropped the
  fixture seeds no photo row and asserts `photos == []` — no generic-cache row, unknown bike → 200 empty),
...
Allegro search was refused). No row has photos (K2 — none stored by design; the scrape was removed after this run).

### Round 3 — after the photo scrape was removed (2026-09-26, same day)

The user dropped the Allegro photo scrape after reading round 1–2 ("nie ma co marnować tokenów"): `searcher/app/allegro_image_fetcher.py`
deleted, `find_allegro_offers` returns straight after the CLI run, `case_allegro` seeds no photo row and asserts `photos == []`.
Re-verified on the restarted searcher (`searcher2.log`):

| Check | Result |
|---|---|
| `cd backend && pytest` | 22 passed |
| `case_allegro` + `case_allegro_search` against the live backend (:8002) | OK / OK |
| searcher imports without the fetcher; `_to_offers` keeps `photos == []` | OK |
| TC-033-01 re-run on **Trek / Marlin 8** (no stored offers) | **Pass** — "Szukam na Allegro i Decathlon…", `/decathlon/search` 200 instant, `/allegro/search` 200 after **86.7 s** (CLI 85.7 s, no scrape step in the log), 1 allegro.pl row stored (used, `price: ""` → "cena w ofercie" in the Used card, 0 photo rows), `offers_new` 2 → 3, `app_errors=[]` (`tc01_r3_marlin8_new_card_button.png`, `tc01_r3_marlin8_searching.png`, `tc01_r3_marlin8_after_search.png`). The New card ends in "Nie znaleziono ofert" because the only listing found is used — expected. |
