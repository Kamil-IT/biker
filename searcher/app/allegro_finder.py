...
Same user message and the same ≤ 3 offers, but NOT the same prompt and NO
photo scrape: allegro.pl answers HTTP 403 (DataDome) to every automated fetch
— the CLI's WebFetch and Chromium alike — so the SDK-era prompt ("open the
listing, open the offer") either burned the whole 300 s CLI timeout or gave up
with no offers, and the old Playwright fetcher could not open a single offer
page (probes 2026-09-26). prompts/bike_offer_allegro.md is a CLI-tuned rewrite
of the same role/rules/output that works from WebSearch results only: listing
URLs, price and condition come from the result title/snippet (a price the
snippet does not show stays ""). Photos were dropped altogether on the user's
decision — like Decathlon, `photos` is always []. Probes with the new prompt:
Kross Level 3.0 → 3 offers in 67 s, Trek Marlin 4 → 3 offers in 75 s. The
result is stored under source 'allegro.pl' — the backend's /v1/bike/allegro is
now a pure DB read.
"""
import asyncio
import logging
...
# would otherwise be stored as an "offer" and linked from the UI.
...
def _to_offers(items: list) -> list[BikeOffer]:
    ...
    showed one (the UI then says "cena w ofercie"). `photos` are always []
    (nothing scrapes Allegro — its pages answer 403) and `city` is None (a
    shop listing, the prompt never asks for one).
    """
...
async def find_allegro_offers(company: str, model: str) -> tuple[list[BikeOffer], str]:
    """One CLI search on allegro.pl for the bike — no photo scrape.
